...
- **Decision history:** Fix documented inline in route.ts (undated in-code, but part of the STLY-basis rework referenced by `kpiForecastPace`/`kpiConfirmedStly` comments, 2026-07-14 era).
...
- **Caveats/gaps:** The numerator/denominator status-set mismatch (30-only vs 20+30) noted above is NOT flagged anywhere in code comments — see §10.3.
- **Decision history:** Reused directly for `EXEC_PACE.vsStly` (2026-07-15).
